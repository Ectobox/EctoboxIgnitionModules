...
| **Schedule** _(coming soon)_ | A modern equipment schedule / planner — a drag-first alternative to the built-in Equipment Schedule. Drag jobs between swim lanes (with per-lane accept rules), resize and multi-select, drop objects onto the timeline to create cards, and dress cards with badges, a progress bar, a colored icon block, and per-lane or plant-wide state bands. Curated palette, automatic light/dark, horizontal or vertical. | _in progress_ | 🚧 Coming soon |
